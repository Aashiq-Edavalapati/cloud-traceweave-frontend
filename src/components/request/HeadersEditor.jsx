...
export default function HeadersEditor() {
    ...
    const updateHeader = (index, key, value) => {
        if (headers.length === 0 && index === 0) {
            store.updateRequestListConfig(['config', 'headers'], 0, 'key', key);
            store.updateRequestListConfig(['config', 'headers'], 0, 'value', value);
        } else {
            store.updateRequestListConfig(['config', 'headers'], index, key, value);
        }
    };

    const removeHeader = (index) => {
        store.removeRequestListItem(['config', 'headers'], index);
    };

    const toggleActive = (index) => {
        const currentVal = headers[index].active !== false;
        store.updateRequestListConfig(['config', 'headers'], index, 'active', !currentVal);
    };

    const addNewHeader = () => {
        const newIndex = headers.length;
        store.updateRequestListConfig(['config', 'headers'], newIndex, 'key', '');
    };
